use crate::store::{RedisClient, SGPool};
use crate::types::*;
use anyhow::Context;
...
pub trait PaymentMethodsInterface{
...
fn update_attempt_cql() -> String {
    "UPDATE payments.payment_attempts set connector_metadata = ? WHERE payment_id = ? AND merchant_id = ? AND attempt_id = ?; IF EXISTS"
      .to_owned()
}

fn update_intent_cql() -> String {
    "UPDATE payments.payment_intents set status = ? WHERE payment_id = ? AND merchant_id = ? IF status != ?;"
        .to_string()
}
...
impl PaymentIntentInterface for crate::store::SGPool {
    ...
    async fn update_intent<'a>(
        ...
        let query = stargate_grpc::Query::builder()
                        ...
                        .bind(("SUCCESS",payment_id, "kaps", "SUCCESS"))
                        .build();
        let mut client = self.pool.get().await.unwrap();
...
#[cfg(feature = "astra")]
#[async_trait::async_trait]
impl PaymentMethodsInterface for SGPool{
    async fn create_payment_method(&self, customer_id : String, payment_method_id : String)
        -> Result<String, Box<dyn std::error::Error>>{
            let query = stargate_grpc::Query::builder()
                .keyspace("payments")
            ...
            let pm = PaymentMethods::new(customer_id, payment_method_id)?;
            let up_query = pm.bind_statement(query)?.build();

            let mut client = self.pool.get().await.unwrap();
            crate::utils::time_wrapper(client.execute_query(up_query), "payment_method", "CREATE", None).await?;
