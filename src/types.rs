...
impl PaymentMethods{
    ...
    pub fn bind_statement(self, query : stargate_grpc::query::QueryBuilder)
        ...
        Ok(query
            ...
            // .bind_ith(6, self.issuer_name)
            // .bind_ith(7, self.issuer_country)
            // .bind_ith(8, self.payer_country)
            // .bind_ith(9, self.is_stored)
            // .bind_ith(10, self.swift_code)
            // .bind_ith(11, self.direct_debit_token)
            // .bind_ith(12, self.created_at)
            // .bind_ith(13, self.last_modified)
            // .bind_ith(14, self.payment_method)
            // .bind_ith(15, self.payment_method_type)
            // .bind_ith(16, self.payment_method_issuer)
            // .bind_ith(17, self.payment_method_issuer_code)
            // .bind_ith(18, self.metadata)
            // .bind_ith(19, self.payment_method_data)
            // .bind_ith(20, self.locker_id)
            // .bind_ith(21, self.last_used_at)
            // .bind_ith(22, self.connector_mandate_details)
            // .bind_ith(23, self.customer_acceptance)
            // .bind_ith(24, self.status)
            // .bind_ith(25, self.network_transaction_id)
